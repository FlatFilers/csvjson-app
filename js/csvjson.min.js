...
// csv2json.js
CSVJSON.csv2json=function(){var errorDetectingSeparator="We could not detect the separator.",errorEmpty="Please upload a file or type in something.",errorEmptyHeader="Could not detect header. Ensure first row cotains your column headers.";var uploadUrl="/csv2json/upload",charMap={comma:',',semiColon:';',tab:'\t'};var $file=$('#fileupload'),$fileLabel=$('.fileinput-button>label'),fileLabelHtml=$fileLabel.html(),$separator=$('input[type=radio][name="separator"]'),$csv=$('#csv'),$json=$('#json'),$clear=$('a.clear'),$convert=$('#convert');function getSeparator(csv){var userSpecified=$separator.filter(':checked').val(),separator=charMap[userSpecified];if(separator)return separator;var counts={},keyMax;_.each(charMap,function(c,k){var re=new RegExp(c,'g');counts[k]=(csv.match(re)||[]).length;keyMax=!keyMax||counts[k]>counts[keyMax]?k:keyMax;});return keyMax?charMap[keyMax]:undefined;}
$file.fileupload({url:uploadUrl,progress:function(e,data){var progress=parseInt(data.loaded/data.total*100,10);$fileLabel.text(progress+'%');},success:function(result){$fileLabel.html(fileLabelHtml);$csv.val(result);},fail:function(e,data){$fileLabel.html(fileLabelHtml);}});$clear.click(function(e){e.preventDefault();$(this).siblings('textarea').val('');return false;});$convert.click(function(e){var csv=_.trim($csv.val());if(csv.length==0)throw errorEmpty;var separator=getSeparator(csv);if(!separator)throw errorDetectingSeparator;console.log('separator',separator);var lines=_.lines(csv);if(lines.length==0)throw errorEmpty;var keys=_.words(lines.shift(),separator);if(keys.length==0)throw errorEmptyHeader;var json=[];for(var l=0;l<lines.length;l++){var row={};var items=_.words(lines[l],separator);for(var i=0;i<keys.length;i++){var value=items[i]-0;row[keys[i]]=isNaN(value)?items[i]:value;}
json.push(row);}
$json.val(JSON.stringify(json,null,2));});};
// json_beautifier.js
CSVJSON.json_beautifier=function(){var errorEmpty="Please upload a file or type in something.";var uploadUrl="/json_beautifier/upload";var $file=$('#fileupload'),$fileLabel=$('.fileinput-button>label'),fileLabelHtml=$fileLabel.html(),$json=$('#json'),$result=$('#result'),$clear=$('a.clear'),$convert=$('#convert');$file.fileupload({url:uploadUrl,progress:function(e,data){var progress=parseInt(data.loaded/data.total*100,10);$fileLabel.text(progress+'%');},success:function(result){$fileLabel.html(fileLabelHtml);$csv.val(result);},fail:function(e,data){$fileLabel.html(fileLabelHtml);}});$clear.click(function(e){e.preventDefault();$(this).siblings('textarea').val('');return false;});function reviver(key,value){var number=value-0;return isNaN(number)?value:number;}
$convert.click(function(e){var space='\t',dropQuotesOnKeys=$('#drop-quotes-on-keys').is(':checked');var json=_.trim($json.val());if(json.length==0)throw errorEmpty;var object=JSON3.parse(json,reviver,space);var result=JSON3.stringify(object,null,space,dropQuotesOnKeys);$result.val(result);});};
// main.js
$(document).ready(function(){_.mixin(_.str.exports());var fn=CSVJSON[CSVJSON.page];if(typeof(fn)!=='function')throw"Module "+CSVJSON.page+" not found.";CSVJSON[CSVJSON.page]();});
