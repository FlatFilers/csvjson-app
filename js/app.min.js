...
localStorage['DataCleanCode'+postfix]=this.get('code');},clearLocalStorage:function(){var postfix=this.id?('_'+this.id):'';delete localStorage['DataCleanInputOptions'+postfix];delete localStorage['DataCleanInputText'+postfix];delete localStorage['DataCleanCode'+postfix];},clearDataOptionsCode:function(){var postfix=this.id?('_'+this.id):'';delete localStorage['DataCleanInputOptions'+postfix];localStorage['DataCleanInputText'+postfix]='Column\r\nCopy and paste data here';localStorage['DataCleanCode'+postfix]=bareCode;},clearData:function(){var postfix=this.id?('_'+this.id):'';localStorage['DataCleanInputText'+postfix]='Column\r\nCopy and paste data here';},clearCode:function(){var postfix=this.id?('_'+this.id):'';localStorage['DataCleanCode'+postfix]=bareCode;}});}.call(this));
// js/src/dataclean-data.js
(function(){Backbone.DataView=Backbone.View.extend({template:_.template(`<div class="row"><div class="col-md-9"><h4>Input<small class="pull-right"><div class="form-inline"><div class="form-group"><div class="checkbox"data-toggle="tooltip"data-placement="top"title="Auto-detect column header as the first row with non-empty cells. Ignore previous rows."><label class=""><input type="checkbox"name="autoDetectHeader"<%=autoDetectHeader?'checked':''%>/>Auto-detect header</label></div></div><div class="form-group">&nbsp;&nbsp;<button class="btn btn-default btn-xs text-danger clear-data"data-toggle="tooltip"data-placement="top"title="Clear input data to start from scratch.">Clear</button></div></div></small></h4><table class="table table-bordered backgrid input"></table><h4>Output<small class="pull-right"><div class="btn-toolbar"><a class="btn btn-primary btn-xs pull-right download"download="dataclean.csv"target="_self"title="Save as dataclean.csv"><i class="glyphicon glyphicon-download"></i>Download CSV</a><button class="btn btn-primary btn-xs pull-right copy"><i class="glyphicon glyphicon-share"></i>Copy to clipboard</button></div></small></h4><table class="table table-bordered backgrid output"></table></div><div class="col-md-3"><h4>Instructions</h4><p><strong>Paste(Ctrl+v)your Excel or Google Sheets data here.</strong></p><p>Then go to Clean&amp;Transform to modify the JavaScript<code>process</code>function.</p><p>Come back afterwards to copy the output.</p><br/><h4>Auto-save</h4><p>Your input data and code is auto-saved to local storage.</p><p>Click on the<strong><i class="glyphicon glyphicon-link"></i>Save</strong>link to persist to server in order to share with colleagues.</p><hr/><div class="btn-toolbar"><button class="btn btn-default btn-xs text-danger clear-local-storage"data-toggle="tooltip"data-placement="bottom"title="<%= id ? 'Reverts data, options and code changes to the last version saved to the server.' : 'Resets data, options and code to the original example.' %>"><%=id?'Reset to server':'Reset to example.'%></button><button class="btn btn-default btn-xs text-danger clear-data-options-code"data-toggle="tooltip"data-placement="bottom"title="Clear data and code to start from scratch.">Clear data and code</button></div></div></div>`),events:{'change input[name=autoDetectHeader]':'onChangeAutoDetectOption','click button.clear-local-storage':'onClickClearLocalStorage','click button.clear-data-options-code':'onClickClearDataOptionsCode','click button.clear-data':'onClickClearData','click button.copy':'copy','click a.download':'onClickDownload'},initialize:function(options){this.store=options.store;this.inputCollection=options.inputCollection;this.outputCollection=options.outputCollection;this.codeView=options.codeView;this.listenTo(this.store,'change:id',this.render);this.listenTo(this.outputCollection,'reset',this.render);$(document).on('paste',this.onPaste.bind(this));_.defer(function(){this.parsePastedText();}.bind(this));},onChangeAutoDetectOption:function(e){var options={autoDetectHeader:this.$('input[name=autoDetectHeader]').is(':checked')};this.store.set({options:options});this.parsePastedText();},onClickClearLocalStorage:function(){this.store.clearLocalStorage();window.location.reload();},onClickClearDataOptionsCode:function(){this.store.clearDataOptionsCode();window.location.reload();},onClickClearData:function(){this.store.clearData();window.location.reload();},copy:function(){_.copyToClipboard(this.outputCollection.toCSV('\t'));this.$('button.copy').yes();},onClickDownload:function(){this.$('a.download').yes();},parsePastedText:function(){this.inputCollection.fullCollection.reset();var options=this.store.get('options');var lines=(this.store.get('text')||'').split('\n');var columns=[];var maxCols=0;var rowWithColumnHeaders;for(var r=0;r<lines.length;r++){var items=lines[r].split('\t');if(items.length>maxCols)maxCols=items.length;if(options.autoDetectHeader&&items.length==_.compact(items).length){rowWithColumnHeaders=r;break;}}
...
(function(){Backbone.CodeView=Backbone.View.extend({template:_.template(`<div class="row"><div class="col-md-6"><h4><span>JavaScript</span><div class="form-inline pull-right"><button class="btn btn-xs btn-success run">Run<em>(Ctrl+R)</em></button><button class="btn btn-xs btn-danger stop hidden"><i class="glyphicon glyphicon-refresh glyphicon-spin"></i>Stop</button>&nbsp;&nbsp;<button class="btn btn-xs btn-default text-danger clear-code"data-toggle="tooltip"data-placement="top"title="Clear code to start from scratch.">Clear</button></div></h4><textarea class="code"><%=code%></textarea></div><div class="col-md-3"><h4>Input<small>2 first rows</small></h4><textarea class="input"><%=input%></textarea></div><div class="col-md-3"><h4>Output<small>2 first rows</small></h4><textarea class="output"><%=output%></textarea></div></div>`),events:{'click button.run':'run','click button.stop':'stop','click button.clear-code':'onClickClearCode'},initialize:function(options){this.store=options.store;this.inputCollection=options.inputCollection;this.outputCollection=options.outputCollection;this.error=undefined;this.workerErrors=[];this.listenTo(this.inputCollection,'ready',this.run);this.listenTo(this.outputCollection,'reset',this.render);this.$tab=$(document).find('a[role=tab][href=#tab-code]');this.$tab.on('shown.bs.tab',this.render.bind(this));$(window).on('keydown',this.onKeydownCtrlR.bind(this));},onKeydownCtrlR:function(e){if($('#tab-code').is(':visible')&&(e.ctrlKey||e.metaKey)&&String.fromCharCode(event.which).toLowerCase()=='r'){e.preventDefault();this.run();}},onClickClearCode:function(){this.store.clearCode();localStorage.DataCleanShowCodePage=true;window.location.reload();},run:function(){this.$('button.run').addClass('hidden');this.$('button.stop').removeClass('hidden');this.workerErrors=[];this.sandbox=new Worker('/js/src/sandbox.js');this.sandbox.addEventListener('message',function(e){this.error=undefined;for(var i=0;i<e.data.length;i++)e.data[i].__Row=i;this.outputCollection.fullCollection.reset(e.data);this.stop();}.bind(this));this.sandbox.addEventListener('error',function(e){this.error=e.message;this.workerErrors=[{message:e.message,lineno:e.lineno}];this.outputCollection.fullCollection.reset();this.stop();}.bind(this));var input=this.inputCollection.fullCollection.reduce(function(l,m){if(!m.get('__Error'))l.push(_.omit(m.toJSON(),'__Error','__Row'));return l;},[]);var code=this.store.get('code')+'\nprocess('+JSON.stringify(input)+', '+JSON.stringify(Backbone.InputModel.getColumns())+');';this.sandbox.postMessage(code);},stop:function(){this.sandbox.terminate();this.$('button.stop').addClass('hidden');this.$('button.run').removeClass('hidden');},toRender:function(){var i=0;var input=this.inputCollection.fullCollection.reduce(function(l,m){if(!m.get('__Error')){if(i<2)l.push(_.omit(m.toJSON(),'__Row','__Error'));i+=1;}
return l;},[]);var output=this.outputCollection.fullCollection.reduce(function(l,m,i){if(i<2)l.push(_.omit(m.toJSON(),'__Row'));return l;},[]);return{code:this.store.get('code'),input:JSON2_mod.stringify(input,null,2),output:JSON2_mod.stringify(output,null,2),error:this.error};},renderWorkerErrors:function(){if(!this.codeEditor||!this.$el.hasClass('active in'))return;this.codeEditor.clearGutter('worker-error');var lineCount=$(this.codeEditor.getWrapperElement()).find('.CodeMirror-code > div:last-child div.CodeMirror-gutter-elt').text();for(var i=0;i<this.workerErrors.length;i++){var error=this.workerErrors[i];var lineno=error.lineno-1;if(lineno>=lineCount)lineno=0;var $marker=$('<div class="CodeMirror-lint-marker-error worker-error" data-toggle="tooltip" data-placement="right" data-container="body" title="'+error.message+'"></div>');this.codeEditor.getDoc().setGutterMarker(lineno,'worker-error',$marker[0]);$marker.tooltip();}},render:function(){if(this.error){this.$tab.html('<small class="text-danger" title="'+this.error+'"><i class="glyphicon glyphicon-exclamation-sign"></i></small> Clean &amp; Transform');}else{this.$tab.html('Clean &amp; Transform');}
if(!this.$el.hasClass('active in'))return this;var data=this.toRender();if(this.$el.is(':empty')){this.$el.html(this.template(data));this.$('[data-toggle="tooltip"]').tooltip();this.codeEditor=CodeMirror.fromTextArea(this.$('textarea.code')[0],{lineNumbers:true,mode:'text/javascript',styleActiveLine:true,gutters:['CodeMirror-lint-markers','worker-error'],lint:true,indentUnit:2});this.codeEditor.setSize('100%','100%');this.codeEditor.on('change',function(editor){this.store.set({code:editor.getDoc().getValue()});}.bind(this));this.inputEditor=CodeMirror.fromTextArea(this.$('textarea.input')[0],{mode:'application/json',indentUnit:2});this.inputEditor.setSize('100%','100%');this.outputEditor=CodeMirror.fromTextArea(this.$('textarea.output')[0],{mode:'application/json',indentUnit:2});this.outputEditor.setSize('100%','100%');this.run();return this;}
...
// js/src/dataclean.js
APP.dataclean=function(){function go(){var inputCollection=window.inputCollection=new Backbone.InputCollection();var outputCollection=window.outputCollection=new Backbone.OutputCollection();var store=window.store=new Backbone.DataStore(_.extend({},APP.data,{id:APP.id||null}));var codeView=new Backbone.CodeView({el:$('#tab-code'),inputCollection:inputCollection,outputCollection:outputCollection,store:store}).render();var dataView=new Backbone.DataView({el:$('#tab-data'),inputCollection:inputCollection,outputCollection:outputCollection,store:store,codeView:codeView}).render();}
if(localStorage.DataCleanShowCodePage){delete localStorage.DataCleanShowCodePage;$('.nav-tabs a[href="#tab-code"]').tab('show');}
