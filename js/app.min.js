...
var result=JSON.stringify(json,null,$minify.is(':checked')?undefined:2);$result.removeClass('error').val(result).change();});APP.start({$convert:$convert,$clear:$clear,$saveElements:$('input.save, textarea.save'),upload:{$file:$file,url:uploadUrl,$textarea:$csv}});};
// js/src/json2csv.js
APP.json2csv=function(){var uploadUrl='/json2csv/upload',sepMap={comma:',',semiColon:';',tab:'\t'},$file=$('#fileupload'),$separator=$('select[name=separator]'),$json=$('#json'),$result=$('#result'),$clear=$('#clear, a.clear'),$convert=$('#convert, a.convert'),$flatten=$('#flatten'),$output_csvjson_variant=$('#output_csvjson_variant');$convert.click(function(e){e.preventDefault();var json=_.trim($json.val());if(json.length==0)err(errorEmpty);var options={separator:sepMap[$separator.find('option:selected').val()],flatten:$flatten.is(':checked'),output_csvjson_variant:$output_csvjson_variant.is(':checked')};var result;try{result=CSVJSON.json2csv(json,options);}catch(error){APP.reportError($result,error);return false;}
$result.removeClass('error').val(result).change();});APP.start({$convert:$convert,$clear:$clear,$saveElements:$('input.save, textarea.save'),upload:{$file:$file,url:uploadUrl,$textarea:$result}});};
// js/src/sql2json.js
...
(function(){var defaultText=`Match\tDate\tResult\r\nTwins vs Yankees\t17-10-03\t4-8\r\nRockies vs Diamondbacks\t17-10-04\t8-11\r\nRed Sox vs Astros\t17-10-05\t2-8`;var defaultCode=`function process(input,columns){var output=[];input.forEach(function(inRow,r){var outRow={};var teams=inRow.Match.split('vs');outRow.Home=teams[0].trim();outRow.Away=teams[1].trim();var date=moment(inRow.Date,'YY-MM-DD');outRow.Date=date.format('MMM Do YYYY');var scores=inRow.Result.split('-');outRow['Home Score']=parseInt(scores[0].trim(),10);outRow['Away Score']=parseInt(scores[1].trim(),10);outRow.Winner=outRow['Home Score']>outRow['Away Score']?outRow.Home:outRow.Away;if(outRow['Home Score']==outRow['Away Score'])outRow.Winner='Tie';output.push(outRow);});return output;}`;Backbone.DataStore=Backbone.Model.extend({defaults:{id:null,date:null,options:{autoDetectHeader:true},text:defaultText,code:defaultCode},urlRoot:function(){return APP.baseUrl()+'/save';},initialize:function(attrs,options){var postfix=attrs&&attrs.id?('_'+attrs.id):'';var data={};if(localStorage['DataCleanInputOptions'+postfix]!==undefined)
try{data.options=JSON.parse(localStorage['DataCleanInputOptions'+postfix]);}catch(err){}
if(localStorage['DataCleanInputText'+postfix]!==undefined)
data.text=localStorage['DataCleanInputText'+postfix];if(localStorage['DataCleanCode'+postfix]!==undefined)
data.code=localStorage['DataCleanCode'+postfix];this.set(data);this.on('change',this.saveToLocalStorage);},saveToLocalStorage:function(){var postfix=this.id?('_'+this.id):'';if(this.hasChanged('options'))
localStorage['DataCleanInputOptions'+postfix]=JSON.stringify(this.get('options'));if(this.hasChanged('text'))
localStorage['DataCleanInputText'+postfix]=this.get('text');if(this.hasChanged('code'))
localStorage['DataCleanCode'+postfix]=this.get('code');},clearLocalStorage:function(){var postfix=this.id?('_'+this.id):'';delete localStorage['DataCleanInputOptions'+postfix];delete localStorage['DataCleanInputText'+postfix];delete localStorage['DataCleanCode'+postfix];}});}.call(this));
// js/src/dataclean-input.js
(function(){Backbone.InputView=Backbone.View.extend({template:_.template(`<div class="row"><div class="col-md-3"><p><strong>Paste(Ctrl+v)your Excel or Google Sheets data here.</strong></p><br/><h4>Options</h4><form><div class="checkbox"><label class=""><input type="checkbox"name="autoDetectHeader"<%=autoDetectHeader?'checked':''%>/>Auto-detect column header as the first row with non-empty cells.Ignore previous rows.</label></div></form><br/><h4>Auto-save</h4><p>Your input data and code is auto-saved to local storage.</p><p>Click on the<strong><i class="glyphicon glyphicon-link"></i>Save</strong>link to persist to server in order to share with colleagues.</p><br/><p><a class="btn btn-sm btn-default text-danger clear-local-storage"href="#"><i class="glyphicon glyphicon-trash"></i>Clear local storage</a><br/><small><%=id?'Reverts changes to the last version saved to the server.':'Resets to the original example.'%></small></p></div><div class="col-md-9"><table class="table table-bordered backgrid"></table></div></div>`),events:{'change input[type=checkbox]':'onChangeOption','click a.clear-local-storage':'onClickClear'},initialize:function(options){this.store=options.store;this.listenTo(this.store,'change:id',this.render);$(document).on('paste',this.onPaste.bind(this));_.defer(function(){this.parsePastedText();}.bind(this));},onChangeOption:function(e){var options={autoDetectHeader:false};var form=this.$('form').serializeObject();if(form.autoDetectHeader)options.autoDetectHeader=true;this.store.set({options:options});this.parsePastedText();},onClickClear:function(e){e.preventDefault();this.store.clearLocalStorage();window.location.reload();},parsePastedText:function(){this.collection.reset();var options=this.store.get('options');var lines=(this.store.get('text')||'').split('\r\n');var columns=[];var maxCols=0;var rowWithColumnHeaders;for(var r=0;r<lines.length;r++){var items=lines[r].split('\t');if(items.length>maxCols)maxCols=items.length;if(options.autoDetectHeader&&items.length==_.compact(items).length){rowWithColumnHeaders=r;break;}}
if(!options.autoDetectHeader||rowWithColumnHeaders===undefined){for(var i=0;i<maxCols;i++)columns.push(i+'');}else{var columns=_.map(lines[rowWithColumnHeaders].split('\t'),function(s){return s.trim();});}
Backbone.InputModel.setColumns(columns);_.each(lines,function(row,r){var error;if(options.autoDetectHeader&&rowWithColumnHeaders!==undefined&&rowWithColumnHeaders>=r){error='Header row and previous rows will not be processed.';}
var model=new Backbone.InputModel({__Row:r,__Error:error});_.each(row.split('\t'),function(cell,c){var value=cell.trim();var name=columns[c];model.set(name,value);}.bind(this));this.collection.add(model);}.bind(this));this.collection.trigger('ready');this.render();},onPaste:function(e){if(!this.$el.hasClass('active'))return;if(!e.originalEvent||!e.originalEvent.clipboardData||!e.originalEvent.clipboardData.items)return;var data=_.findWhere(e.originalEvent.clipboardData.items,{type:'text/plain'});if(!data)return;data.getAsString(function(text){this.store.set({text:text.trim('\r\n')});this.parsePastedText();}.bind(this));},toRender:function(){return _.extend({id:this.store.id,},this.store.get('options'));},render:function(){var data=this.toRender();this.$el.html(this.template(data));var columns=[{name:'__Row',label:'',cell:Backgrid.RowCountCell,editable:false}];_.each(Backbone.InputModel.getColumns(),function(col){columns.push({name:col,label:col,cell:Backgrid.ReportingCell,editable:false});});new Backgrid.Grid({el:this.$('table'),collection:this.collection,columns:columns}).render();return this;}});Backbone.InputModel=Backbone.Model.extend({defaults:{__Row:null,__Error:null},},{setColumns:function(columns){var defaults={__Row:null,__Error:null};_.each(columns,function(col){defaults[col]=null;});Backbone.InputModel.prototype.defaults=defaults;},getColumns:function(){return _.without(_.keys(Backbone.InputModel.prototype.defaults),'__Row','__Error');}});Backbone.InputCollection=Backbone.Collection.extend({model:Backbone.InputModel});Backgrid.RowCountCell=Backgrid.Cell.extend({className:'reporting-cell',render:function(){this.$el.empty();var columnName=this.column.get('name');var value=this.model.get(columnName)+1;this.$el.html(value);var error=this.model.get('__Error');if(error)this.$el.append('&nbsp;<i class="glyphicon glyphicon-warning-sign text-danger" title="'+(error+'').replace(/"/g,"'")+'"></i>');this.delegateEvents();return this;}});Backgrid.ReportingCell=Backgrid.Cell.extend({className:'reporting-cell',render:function(){this.$el.empty();var columnName=this.column.get('name');var rawValue=this.model.get(columnName);var formattedValue=this.formatter.fromRaw(rawValue,this.model);this.$el.html(formattedValue);if(this.model.errorModel){var error=this.model.errorModel.get(columnName);if(error)this.$el.prepend('<i class="fa fa-fw fa-warning text-danger" title="'+(error+'').replace(/"/g,"'")+'"></i>&nbsp;');}
this.delegateEvents();return this;}});$.fn.serializeObject=function(){var o={};var a=this.serializeArray();$.each(a,function(){if(o[this.name]!==undefined){if(!o[this.name].push){o[this.name]=[o[this.name]];}
o[this.name].push(this.value||'');}else{o[this.name]=this.value||'';}});return o;};}.call(this));
// js/src/dataclean-code.js
(function(){Backbone.CodeView=Backbone.View.extend({template:_.template(`<div class="row"><div class="col-md-6"><h4><span>JavaScript</span><button class="btn btn-xs btn-success pull-right run">Run<em>(Ctrl+R)</em></button><button class="btn btn-xs btn-danger pull-right stop hidden"><i class="glyphicon glyphicon-refresh glyphicon-spin"></i>Stop</button></h4><textarea class="code"><%=code%></textarea></div><div class="col-md-3"><h4>Input<small>2 first rows</small></h4><textarea class="input"><%=input%></textarea></div><div class="col-md-3"><h4>Output<small>2 first rows</small></h4><textarea class="output"><%=output%></textarea></div></div>`),events:{'click button.run':'run','click button.stop':'stop'},initialize:function(options){this.store=options.store;this.inputCollection=options.inputCollection;this.outputCollection=options.outputCollection;this.error=undefined;this.workerErrors=[];this.listenTo(this.inputCollection,'ready',this.run);this.listenTo(this.outputCollection,'reset',this.render);$(document).find('a[role=tab][href=#tab-code]').on('shown.bs.tab',this.render.bind(this));$(window).on('keydown',this.onKeydownCtrlR.bind(this));},onKeydownCtrlR:function(e){if($('#tab-code').is(':visible')&&(e.ctrlKey||e.metaKey)&&String.fromCharCode(event.which).toLowerCase()=='r'){e.preventDefault();this.run();}},run:function(){this.$('button.run').addClass('hidden');this.$('button.stop').removeClass('hidden');this.workerErrors=[];this.sandbox=new Worker('/js/src/sandbox.js');this.sandbox.addEventListener('message',function(e){this.error=undefined;for(var i=0;i<e.data.length;i++)e.data[i].__Row=i;this.outputCollection.reset(e.data);this.stop();}.bind(this));this.sandbox.addEventListener('error',function(e){this.error=e.message;this.workerErrors=[{message:e.message,lineno:e.lineno}];this.outputCollection.reset();this.stop();}.bind(this));var input=this.inputCollection.reduce(function(l,m){if(!m.get('__Error'))l.push(_.omit(m.toJSON(),'__Error','__Row'));return l;},[]);var code=this.store.get('code')+'\nprocess('+JSON.stringify(input)+', '+JSON.stringify(Backbone.InputModel.getColumns())+');';this.sandbox.postMessage(code);},stop:function(){this.sandbox.terminate();this.$('button.stop').addClass('hidden');this.$('button.run').removeClass('hidden');},toRender:function(){var i=0;var input=this.inputCollection.reduce(function(l,m){if(!m.get('__Error')){if(i<2)l.push(_.omit(m.toJSON(),'__Row','__Error'));i+=1;}
return l;},[]);var output=this.outputCollection.reduce(function(l,m,i){if(i<2)l.push(_.omit(m.toJSON(),'__Row'));return l;},[]);return{code:this.store.get('code'),input:JSON2_mod.stringify(input,null,2),output:JSON2_mod.stringify(output,null,2),error:this.error};},renderWorkerErrors:function(){if(!this.codeEditor||!this.$el.hasClass('active in'))return;this.codeEditor.clearGutter('worker-error');var lineCount=$(this.codeEditor.getWrapperElement()).find('.CodeMirror-code > div:last-child div.CodeMirror-gutter-elt').text();for(var i=0;i<this.workerErrors.length;i++){var error=this.workerErrors[i];var lineno=error.lineno-1;if(lineno>=lineCount)lineno=0;var $marker=$('<div class="CodeMirror-lint-marker-error worker-error" data-toggle="tooltip" data-placement="right" data-container="body" title="'+error.message+'"></div>');this.codeEditor.getDoc().setGutterMarker(lineno,'worker-error',$marker[0]);$marker.tooltip();}},render:function(){if(!this.$el.hasClass('active in'))return this;var data=this.toRender();if(this.$el.is(':empty')){this.$el.html(this.template(data));this.codeEditor=CodeMirror.fromTextArea(this.$('textarea.code')[0],{lineNumbers:true,mode:'text/javascript',styleActiveLine:true,gutters:['CodeMirror-lint-markers','worker-error'],lint:true,indentUnit:2});this.codeEditor.setSize('100%','100%');this.codeEditor.on('change',function(editor){this.store.set({code:editor.getDoc().getValue()});}.bind(this));this.inputEditor=CodeMirror.fromTextArea(this.$('textarea.input')[0],{mode:'application/json',indentUnit:2});this.inputEditor.setSize('100%','100%');this.outputEditor=CodeMirror.fromTextArea(this.$('textarea.output')[0],{mode:'application/json',indentUnit:2});this.outputEditor.setSize('100%','100%');this.run();return this;}
this.inputEditor.getDoc().setValue(data.input);this.outputEditor.getDoc().setValue(this.error||data.output);this.outputEditor.setOption('lineWrapping',!!this.error);this.renderWorkerErrors();return this;}});}.call(this));
// js/src/dataclean-output.js
(function(){Backbone.OutputView=Backbone.View.extend({template:_.template(`<div class="row"><div class="col-md-9"><table class="table table-bordered backgrid"></table></div><div class="col-md-3"><p><strong>Download as CSV or copy data and paste back in Excel or Google Sheets.</strong></p><br/><div class="row"><div class="col-xs-9"><a class="btn btn-primary btn-block download"download="dataclean.csv"target="_self"title="Save as dataclean.csv"><i class="glyphicon glyphicon-download"></i>Download CSV</a><br/><button class="btn btn-primary btn-block copy"><i class="glyphicon glyphicon-share"></i>Copy to clipboard</button></div></div><br/></div></div>`),events:{'click button.copy':'copy','click a.download':'onClickDownload'},initialize:function(options){this.store=options.store;$(document).find('a[role=tab][href=#tab-output]').on('shown.bs.tab',this.render.bind(this));},copy:function(){_.copyToClipboard(this.collection.toCSV('\t'));this.$('button.copy').yes();},onClickDownload:function(){this.$('a.download').yes();},toRender:function(){return{};},render:function(){var data=this.toRender();this.$el.html(this.template(data));var columns=[{name:'__Row',label:'',cell:Backgrid.RowCountCell,editable:false}];if(this.collection.size()){var keys=_.without(this.collection.first().keys(),'__Row');_.each(keys,function(col){columns.push({name:col,label:col,cell:Backgrid.ReportingCell,editable:false});});}
new Backgrid.Grid({el:this.$('table'),collection:this.collection,columns:columns}).render();var $download=this.$('a.download');var csv=escape(this.collection.toCSV(','));$download.attr('href','data:application/json;charset=utf-8,'+csv);if(csv&&csv.length>65400){$download.attr('disabled',true).attr('title','Too large to download. Copy to clipboard instead.');}else if(csv.length==0){$download.attr('disabled',true);$download.attr('title','No data to download. Convert first.');}
return this;}});Backbone.OutputModel=Backbone.Model.extend({defaults:{__Row:null}});Backbone.OutputCollection=Backbone.Collection.extend({model:Backbone.OutputModel,toCSV:function(separator){if(this.size()==0)return'';var keys=_.without(this.first().keys(),'__Row');var data=this.map(function(model){return model.pick(keys);});return _.convert(data,separator);}});$.fn.yes=function(){this.each(function(){var $el=$(this);var classNames=$el.find('.glyphicon').attr('class');$el.find('.glyphicon').removeClass().addClass('glyphicon glyphicon-refresh glyphicon-spin');$el.attr('disabled',true);setTimeout(function(){$el.removeAttr('disabled');$el.find('.glyphicon').removeClass().addClass('glyphicon glyphicon-ok');setTimeout(function(){$el.find('.glyphicon').removeClass().addClass(classNames);},2000);},500);});};_.copyToClipboard=function(str){var $textarea=$('<textarea style="position:absolute;left:-9999px;" readonly></textarea>');$('body').append($textarea);$textarea.val(str);$textarea[0].select();document.execCommand('copy');$textarea.remove();};_.convert=function(data,separator){separator||(separator=',');if(!_.isObject(data))throw'Your JSON must be an array or an object.';if(!_.isArray(data))data=[data];var allKeys=[],allRows=[];for(var i=0;i<data.length;i++){var o=data[i],row={};if(o!==undefined&&o!==null&&(!_.isObject(o)||_.isArray(o)))
