...
(function(){var errorMissingSeparator='Missing separator option.',errorEmpty='Please upload a file or type in something.',errorEmptyHeader='Could not detect header. Ensure first row cotains your column headers.',errorNotAnArray='Your JSON must be an array or an object.',errorItemNotAnObject='Item in array is not an object: {0}',errorHelp="\n\nOH NO! I don't know how to convert that. Help me understand what you want. Click on the button below to report a bug or suggestion.";function convert(json,options){options||(options={});var data=jsonlint.parse(json);if(!_.isObject(data))throw errorNotAnArray+errorHelp;if(!_.isArray(data))data=[data];var separator=options.separator;if(!separator)throw errorMissingSeparator;var flatten=options.flatten||false;var allKeys=[],allRows=[];if(flatten){var merge=function(objects){var out={};for(var i=0;i<objects.length;i++){for(var p in objects[i]){out[p]=objects[i][p];}}
return out;}
var flatten=function(obj,name,stem){var out={};var newStem=(typeof stem!=='undefined'&&stem!=='')?stem+'_'+name:name;if(typeof obj!=='object'){out[newStem]=obj;return out;}
for(var p in obj){var prop=flatten(obj[p],p,newStem);out=merge([out,prop]);}
return out;};data=flatten(data);}
for(var i=0;i<data.length;i++){var o=data[i],row={};if(o!==undefined&&o!==null&&(!_.isObject(o)||_.isArray(o)))
