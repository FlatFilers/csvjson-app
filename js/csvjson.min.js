...
function inlineShortArraysInResult(result){var list=result.split('\n'),i=0,start=null,content=[];while(i<list.length){if(list[i].match(/\[$/)){start=i;}else if(list[i].match(/\],?$/)&&start){var inline=list[start]+content.join(' ')+_.trim(list[i]);if(inline.length<80){list.splice(start,i-start+1,inline);i=start;}
start=null;content=[];}else{if(start)content.push(_.trim(list[i]));}
i+=1;}
return list.join('\n');}
function convert(json,options){if(json.length==0)throw errorEmpty;var space=options.space||2,dropQuotesOnKeys=options.dropQuotesOnKeys||false,dropQuotesOnNumbers=options.dropQuotesOnNumbers||false,inlineShortArrays=options.inlineShortArrays||false;var object=jsonlint.parse(json);if(dropQuotesOnNumbers)walkObjectAndDropQuotesOnNumbers(object);var result=JSON2_mod.stringify(object,null,space,dropQuotesOnKeys);if(inlineShortArrays)result=inlineShortArraysInResult(result);return result;};this.CSVJSON||(this.CSVJSON={});this.CSVJSON.json_beautifier=convert;}).call(this);
