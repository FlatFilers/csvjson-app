...
function convert(csv,options){options||(options={});if(csv.length==0)throw errorEmpty;var separator=options.separator||detectSeparator(csv);if(!separator)throw errorDetectingSeparator;var lines=_.lines(csv);if(lines.length==0)throw errorEmpty;var keys=_.words(lines.shift(),separator);if(keys.length==0)throw errorEmptyHeader;keys=_.map(keys,function(key){return _(key).chain().trim().trim('"').value();});var json=[];for(var l=0;l<lines.length;l++){var row={};var items=_.words(lines[l],separator);for(var i=0;i<keys.length;i++){var value=_(items[i]).chain().trim().trim('"').value(),number=value-0;row[keys[i]]=isNaN(number)?value:number;}
json.push(row);}
return json;};this.CSVJSON||(this.CSVJSON={});this.CSVJSON.csv2json=convert;}).call(this);
// js/csvjson/sql2json.js
(function(){var errorEmpty="Please upload a file or type in something.";function convert(sql){if(sql.length==0)throw errorEmpty;sql=sql.replace(/(?:\/\*(?:[\s\S]*?)\*\/)|(?:([\s;])+\/\/(?:.*)$)/gm,'$1').replace(/^--.*[\r\n]/gm,"").replace(/^\s*[\r\n]/gm,"").replace(/;\s*[\r\n]/gm,";;").replace(/[\r\n]/gm,"").replace(/;;/gm,";\n");var lines=_.lines(sql);if(lines.length==0)throw errorEmpty;var tables={},l,line;try{for(l=0;l<lines.length;l++){line=lines[l],words=_.words(line);if(!words.length)continue;if(words.length>=3&&words[0].toUpperCase()=='CREATE'&&words[1].toUpperCase()=='TABLE'){var name=_.trim(words[2],"`'\"");tables[name]={header:[],values:[]};var values=_(line).chain().strRight("(").strLeftBack(")").words(",").value();tables[name].header=_.reduce(values,function(result,value){var words=_.words(value);if(!words.length)
...
throw"No values found for table "+name;tables[name].values.push(_.map(values,function(value){return _.trim(value," `'\"");}));}}}catch(error){throw"Error: "+error+"\n..."+line;}
