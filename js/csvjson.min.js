...
$convert.click(function(e){var space=spaceMap[$('#space').val()],dropQuotesOnKeys=$('#drop-quotes-on-keys').is(':checked'),dropQuotesOnNumbers=$('#drop-quotes-on-numbers').is(':checked');var json=_.trim($json.val());if(json.length==0)err(errorEmpty);var object,result;try{object=jsonlint.parse(json);if(dropQuotesOnNumbers)walkObjectAndDropQuotesOnNumbers(object);result=JSON3.stringify(object,null,space,dropQuotesOnKeys);$result.removeClass('error').val(result);}catch(error){err(error);}});};
