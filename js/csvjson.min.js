...
// js/src/csv2json.js
CSVJSON.csv2json=function(){var errorDetectingSeparator="We could not detect the separator.",errorEmpty="Please upload a file or type in something.",errorEmptyHeader="Could not detect header. Ensure first row cotains your column headers.";var uploadUrl="/csv2json/upload",charMap={comma:',',semiColon:';',tab:'\t'};var $file=$('#fileupload'),$separator=$('input[type=radio][name="separator"]'),$csv=$('#csv'),$json=$('#json'),$clear=$('a.clear'),$convert=$('#convert');function getSeparator(csv){var userSpecified=$separator.filter(':checked').val(),separator=charMap[userSpecified];if(separator)return separator;var counts={},keyMax;_.each(charMap,function(c,k){var re=new RegExp(c,'g');counts[k]=(csv.match(re)||[]).length;keyMax=!keyMax||counts[k]>counts[keyMax]?k:keyMax;});return keyMax?charMap[keyMax]:undefined;}
CSVJSON.bindFileUploadToFillTextarea($file,uploadUrl,$csv);CSVJSON.bindClear($clear);function err(error){CSVJSON.reportError($json,error);return false;}
$convert.click(function(e){var csv=_.trim($csv.val());if(csv.length==0)return err(errorEmpty);var separator=getSeparator(csv);if(!separator)return err(errorDetectingSeparator);console.log('separator',separator);var lines=_.lines(csv);if(lines.length==0)return err(errorEmpty);var keys=_.words(lines.shift(),separator);if(keys.length==0)return err(errorEmptyHeader);var json=[];for(var l=0;l<lines.length;l++){var row={};var items=_.words(lines[l],separator);for(var i=0;i<keys.length;i++){var value=items[i]-0;row[keys[i]]=isNaN(value)?items[i]:value;}
json.push(row);}
var result=JSON.stringify(json,null,2);$json.removeClass('error').val(result);});};
// js/src/json_beautifier.js
CSVJSON.json_beautifier=function(){var errorEmpty="Please upload a file or type in something.";var uploadUrl="/json_beautifier/upload",spaceMap={'tab':'\t','1':1,'2':2,'3':3,'4':4,'.':'.','..':'..'};var $file=$('#fileupload'),$json=$('#json'),$result=$('#result'),$clear=$('a.clear'),$convert=$('#convert');CSVJSON.bindFileUploadToFillTextarea($file,uploadUrl,$json);CSVJSON.bindClear($clear);function err(error){CSVJSON.reportError($result,error);return false;}
function reviver(key,value){var number=value-0;return isNaN(number)?value:number;}
$convert.click(function(e){var space=spaceMap[$('#space').val()],dropQuotesOnKeys=$('#drop-quotes-on-keys').is(':checked'),dropQuotesOnNumbers=$('#drop-quotes-on-numbers').is(':checked');var json=_.trim($json.val());if(json.length==0)err(errorEmpty);var object,result;try{object=jsonlint.parse(json,dropQuotesOnNumbers?reviver:null,space);result=JSON3.stringify(object,null,space,dropQuotesOnKeys);$result.removeClass('error').val(result);}catch(error){err(error);}});};
// js/src/jquery.cache-inputs.js
(function($){$.fn.CacheInputs=function(options){var settings=$.extend({key:'cache-inputs',inputs:['input[type=text]','input[type=radio]','input[type=checkbox]','select']},options);function on_change(event){var $input=$(event.target),key=settings.key,id=$input.attr('id'),data=JSON.parse(localStorage[key]);if($input.attr('type')=='radio'||$input.attr('type')=='checkbox'){data[id]=$input.is(':checked');}else{data[id]=$input.val();}
localStorage[key]=JSON.stringify(data);}
return this.each(function(){var element=$(this),selector=settings.inputs.join(', ');if(typeof(Storage)!=="undefined"){var key=settings.key;var data=false;if(localStorage[key]){data=JSON.parse(localStorage[key]);}
if(!data){localStorage[key]=JSON.stringify({});data=JSON.parse(localStorage[key]);}
element.find(selector).change(on_change);element.find(selector).each(function(){if($(this).attr('type')!='submit'){var $input=$(this),id=$input.attr('id'),value=data[id];if(value===undefined)return true;if($input.attr('type')=='radio'||$input.attr('type')=='checkbox'){if(value){$input.attr('checked',true);}else{$input.removeAttr('checked');}}else{$input.val(value);}}});}});};}(jQuery));
// js/src/main.js
$(document).ready(function(){_.mixin(_.str.exports());_.extend(CSVJSON,{reportError:function($textarea,error){$textarea.addClass('error').val(error);},bindClear:function($clear){$clear.click(function(e){e.preventDefault();$(this).siblings('textarea').val('').removeClass('error');return false;});},bindFileUploadToFillTextarea:function($file,uploadUrl,$textarea){var $fileLabel=$file.siblings('label'),fileLabelHtml=$fileLabel.html();$file.fileupload({url:uploadUrl,progress:function(e,data){var progress=parseInt(data.loaded/data.total*100,10);$fileLabel.text(progress+'%');},success:function(result){$fileLabel.html(fileLabelHtml);$textarea.val(result);},fail:function(e,data){$fileLabel.html(fileLabelHtml);}});}});$('.container').CacheInputs({key:CSVJSON.page});var fn=CSVJSON[CSVJSON.page];if(typeof(fn)!=='function')throw"Module "+CSVJSON.page+" not found.";CSVJSON[CSVJSON.page]();});
