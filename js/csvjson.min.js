...
throw"Table "+name+" was not defined in a CREATE TABLE.";var table=tables[name];var values=_(line).chain().strRight("(").strLeftBack(")").words(",").value();if(!values.length)throw"No values found for table "+name;tables[name].values.push(_.map(values,function(value){return _.trim(value," `'\"");}));}
...
throw"Error parsing INSERT INTO statement. No values found after VALUES.";var records=_.trim(words.slice(i).join(" ")).replace(/(\))\s*,\s*(\()/g,"),(").replace(/^\(/,"").replace(/\)$/,"").split("),(");_.each(records,function(str){var values=_.words(str,",");tables[name].values.push(_.map(values,function(value){return _.trim(value," `'\"");}));});}}}catch(error){throw"Error: "+error+"\n..."+line;}
